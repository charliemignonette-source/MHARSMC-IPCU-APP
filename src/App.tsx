...
// Validation queries removed to save quota
  }, [user, profile]);
        ...
        {isQuotaExceeded() && (
          <div className="bg-rose-50 border-b border-rose-200 text-rose-800 px-4 sm:px-8 py-3 text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 font-semibold">
            <div className="flex items-start sm:items-center gap-2">
              ...
              <span>
                <strong>Daily Database Quota Reached.</strong> Operating in limited offline mode using local cache. Real-time updates will automatically sync and restore once the daily limit resets.
              </span>
            </div>
              ...
              <button 
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    alert("Cloud connection is still operating in offline mode. The Firestore daily quota limit has not reset yet (resets daily at midnight PST).");
                  } catch (err) {
                ...
                className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 cursor-pointer shadow-sm"
              >
                Check Connection
              </button>
              <button
                onClick={() => {
                  if (confirm("Are you sure you want to force clear the offline status? If the quota is still exceeded, this warning will reappear on the next save or reload.")) {
                    localStorage.removeItem('firebase_quota_exceeded');
                    window.location.reload();
                  }
                }}
                className="px-3 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 cursor-pointer"
              >
                Force Reset
              </button>
            </div>
