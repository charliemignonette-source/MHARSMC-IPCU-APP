...
export default function Outbreak({ user }: { user: UserProfile | null }) {
  const [selectedReportForValidation, setSelectedReportForValidation] = useState<OutbreakReport | null>(null);
                      ...
                      {user?.role !== "USER" && (
                        <>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setSelectedReportForValidation(report);
                            }}
                            className="p-2 bg-brand-primary/10 text-brand-primary hover:bg-brand-primary/20 rounded-xl transition-colors"
